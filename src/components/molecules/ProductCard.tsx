...
const ProductCard = (props) => {
    ...
    return (
        <div className={`${props.darkMode ? 'bg-Primary_700' : 'border'} relative flex flex-col mobile:gap-2 mx-auto rounded-lg w-full h-fit min-w-[182px] max-w-lg`}>
            {props.quantity === 0 &&
            ...
            <div className='overflow-hidden w-full h-64 rounded-t-lg bg-BackDrop_d_xs'>
                <img src={props.productImg[0] !== '' ? props.productImg[props.productImg.length - 1] : product_placeholder} alt="product_placeholder" className='w-full h-full object-cover cursor-pointer'/>
            </div>
            <div className='w-full flex flex-col gap-5 mobile:gap-2 p-3 mobile:p-1'>
                <div className='flex flex-col gap-4 mobile:gap-2'>
                    <div className={`${props.darkMode ? 'bg-Primary_800' : 'bg-[#f8fafc]'} flex flex-col rounded-lg p-4`}>
                        <p className={`${props.darkMode ? 'text-PrimaryDisabled' : 'text-PrimaryActive'} font-bold text-lg mobile:text-sm`}>{props.name}</p>
                        <p className={`${props.darkMode ? 'text-Primary_200' : 'text-SecondaryAccent6'} font-light text-base mobile:text-xs`}>{props.category}</p>
                    </div>
                    <div className='flex justify-end items-center gap-2'>
                        {/* <Button
                            btnType='button'
                            btnText='Add To Cart'
                            btnStyle={`${props.darkMode ? 'bg-Primary_800 text-Primary hover:bg-BackDrop_d_xs' : 'bg-SecondaryAccent5 text-Primary'} w-fit relative right-0 top-0 z-[22] mb-0 !rounded-lg mobile:!rounded-[5px] truncate flex gap-4 justify-center items-center px-5 py-2 mobile:!px-2 font-normal text-sm mobile:text-xs leading-7 truncate transition ease-in-out duration-250`}
                            onClick={handleAddToCart}
                        /> */}
                        <div className='flex items-baseline gap-1'>
                            <p className='font-bold text-right text-sm mobile:text-xs text-Primary'>
                                {props.currency ? props.currency : 'NaN'}
                            </p>
                            <p className='font-bold text-right text-lg mobile:text-base text-Primary'>
